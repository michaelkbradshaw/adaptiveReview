...
/**
 * Classes to enforce the various access rules that can apply to a areview.
 *
 * @package    mod
 * @subpackage areview
 ...
 * areview, with convinient methods for seeing whether access is allowed.
 *
 * @copyright  2009 Tim Hunt
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class areview_access_manager {
    /** @var areview the areview settings object. */
    protected $areviewobj;
    /** @var int the time to be considered as 'now'. */
    protected $timenow;
    /** @var array of areview_access_rule_base. */
    protected $rules = array();

    /**
     * Create an instance for a particular areview.
     * @param object $areviewobj An instance of the class areview from attemptlib.php.
     *      The areview we will be controlling access to.
     * @param int $timenow The time to use as 'now'.
     * @param bool $canignoretimelimits Whether this user is exempt from time
     *      limits (has_capability('mod/areview:ignoretimelimits', ...)).
     */
    public function __construct($areviewobj, $timenow, $canignoretimelimits) {
        $this->areviewobj = $areviewobj;
        $this->timenow = $timenow;
        $this->rules = $this->make_rules($areviewobj, $timenow, $canignoretimelimits);
    }

    /**
     * Make all the rules relevant to a particular areview.
     * @param areview $areviewobj information about the areview in question.
     * @param int $timenow the time that should be considered as 'now'.
     * @param bool $canignoretimelimits whether the current user is exempt from
     *      time limits by the mod/areview:ignoretimelimits capability.
     * @return array of {@link areview_access_rule_base}s.
     */
    protected function make_rules($areviewobj, $timenow, $canignoretimelimits) {

        $rules = array();
        foreach (self::get_rule_classes() as $ruleclass) {
            $rule = $ruleclass::make($areviewobj, $timenow, $canignoretimelimits);
            if ($rule) {
        ...
        foreach ($superceededrules as $superceededrule) {
            unset($rules['areviewaccess_' . $superceededrule]);
        }
    ...
    protected static function get_rule_classes() {
        return get_plugin_list_with_class('areviewaccess', '', 'rule.php');
    }
     ...
     * settings are stored in the areview table.
     *
     * @param mod_areview_mod_form $areviewform the areview settings form that is being built.
     * @param MoodleQuickForm $mform the wrapped MoodleQuickForm.
     */
    public static function add_settings_form_fields(
            mod_areview_mod_form $areviewform, MoodleQuickForm $mform) {

        foreach (self::get_rule_classes() as $rule) {
            $rule::add_settings_form_fields($areviewform, $mform);
        }
    }
    ...
    public static function get_browser_security_choices() {
        $options = array('-' => get_string('none', 'areview'));
        foreach (self::get_rule_classes() as $rule) {
     ...
     * @param mod_areview_mod_form $areviewform the areview form object.
     * @return array $errors the updated $errors array.
     */
    public static function validate_settings_form_fields(array $errors,
            array $data, $files, mod_areview_mod_form $this) {
    ...
    /**
     * Save any submitted settings when the areview settings form is submitted.
     *
     * Note that the standard plugins do not use this mechanism, becuase all their
     * settings are stored in the areview table.
     *
     * @param object $areview the data from the areview form, including $areview->id
     *      which is the is of the areview being saved.
     */
    public static function save_settings($areview) {

        foreach (self::get_rule_classes() as $rule) {
            $rule::save_settings($areview);
        }
    }
     ...
     */
    protected static function get_load_sql($areviewid, $rules, $basefields) {
        $allfields = $basefields;
        $alljoins = '{areview} areview';
        $allparams = array('areviewid' => $areviewid);

        foreach ($rules as $rule) {
            list($fields, $joins, $params) = $rule::get_settings_sql($areviewid);
            if ($fields) {
                if ($allfields) {
                    $allfields .= ', ';
                }
        ...
        return array("SELECT $allfields FROM $alljoins WHERE areview.id = :areviewid", $allparams);
    }
     ...
     */
    public static function load_settings($areviewid) {
        global $DB;

        $rules = self::get_rule_classes();
        list($sql, $params) = self::get_load_sql($areviewid, $rules, '');
        ...
        foreach ($rules as $rule) {
            $data += $rule::get_extra_settings($areviewid);
        }
    ...
    /**
     * Load the areview settings and any settings required by the access rules.
     ...
     *
     * @param int $areviewid the areview id.
     * @return object mdl_areview row with extra fields.
     */
    public static function load_areview_and_settings($areviewid) {
        global $DB;

        $rules = self::get_rule_classes();
        list($sql, $params) = self::get_load_sql($areviewid, $rules, 'areview.*');
        $areview = $DB->get_record_sql($sql, $params, MUST_EXIST);

        foreach ($rules as $rule) {
            foreach ($rule::get_extra_settings($areviewid) as $name => $value) {
                $areview->$name = $value;
            }
        }
    ...
    /**
     * Provide a description of the rules that apply to this areview, such
     * as is shown at the top of the areview view page. Note that not all
    ...
    /**
     * Whether or not a user should be allowed to start a new attempt at this areview now.
     ...
     * @param int|null $attemptid the id of the current attempt, if there is one,
     *      otherwise null.
     * @return mod_areview_preflight_check_form the form.
     */
    public function get_preflight_check_form(moodle_url $url, $attemptid) {
        return new mod_areview_preflight_check_form($url->out_omit_querystring(),
                array('rules' => $this->rules, 'areviewobj' => $this->areviewobj,
     ...
     * areview. Used, for example, to change the label by the grade displayed on the view page from
     ...
     *
     * @param object $attempt the data from the relevant areview_attempts row.
    ...
    /**
     * @return bolean if this areview should only be shown to students in a popup window.
     */
    ...
    /**
     * Send the user back to the areview view page. Normally this is just a redirect, but
     * If we were in a secure window, we close this window, and reload the view window we came from.
     *
     * This method does not return;
     *
     * @param mod_areview_renderer $output the areview renderer.
    ...
    public function back_to_view_page($output, $message = '') {
        if ($this->attempt_must_be_in_popup()) {
            echo $output->close_attempt_popup($this->areviewobj->view_url(), $message);
            die();
        } else {
            redirect($this->areviewobj->view_url(), $message);
        }
    }

    /**
     * Make some text into a link to review the areview, if that is appropriate.
     *
    ...
    public function make_review_link($attempt, $reviewoptions, $output) {
        ...
        if (in_array($attempt->state, array(areview_attempt::IN_PROGRESS, areview_attempt::OVERDUE))) {
            return $output->no_review_message('');
        }

        $when = areview_attempt_state($this->areviewobj->get_areview(), $attempt);
        $reviewoptions = mod_areview_display_options::make_from_areview(
                $this->areviewobj->get_areview(), $when);

        if (!$reviewoptions->attempt) {
            return $output->no_review_message($this->areviewobj->cannot_review_message($when, true));

        } else {
            return $output->review_link($this->areviewobj->review_url($attempt->id),
