...
/**
 * Base class for the table used by a {@link areview_attempts_report}.
 *
 * @package   mod_areview
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
abstract class areview_attempts_report_table extends table_sql {
    ...
    /** @var object the areview settings for the areview we are reporting on. */
    protected $areview;

    /** @var context the areview context. */
    protected $context;

    /** @var string HTML fragment to select the first/best/last attempt, if appropriate. */
    protected $qmsubselect;

    /** @var object mod_areview_attempts_report_options the options affecting this report. */
    protected $options;
    ...
    /** @var object the questions that comprise this areview.. */
    protected $questions;
     ...
     * @param mod_areview_attempts_report_options $options
     * @param array $groupstudents
     * @param array $students
     * @param array $questions
     * @param moodle_url $reporturl
     */
    public function __construct($uniqueid, $areview, $context, $qmsubselect,
            mod_areview_attempts_report_options $options, $groupstudents, $students,
            $questions, $reporturl) {
    ...
    public function col_fullname($attempt) {
        ...
        return $html . html_writer::empty_tag('br') . html_writer::link(
                new moodle_url('/mod/areview/review.php', array('attempt' => $attempt->attempt)),
                get_string('reviewattempt', 'areview'), array('class' => 'reviewlink'));
    }
    ...
    public function col_state($attempt) {
        if (!is_null($attempt->attempt)) {
            return areview_attempt::state_name($attempt->state);
        } else {
            return  '-';
        }
    }
    ...
    public function col_feedbacktext($attempt) {
        ...
        $feedback = areview_report_feedback_for_grade(
                areview_rescale_grade($attempt->sumgrades, $this->areview, false),
                $this->areview->id, $this->context);
    ...
    public function base_sql($reportstudents) {
        ...
        $fields = $DB->sql_concat('u.id', "'#'", 'COALESCE(areviewa.attempt, 0)') . ' AS uniqueid,';
        ...
        $fields .= '
                areviewa.uniqueid AS usageid,
                areviewa.id AS attempt,
                u.id AS userid,
                u.idnumber,
                u.firstname,
                u.lastname,
                ...
                areviewa.timefinish,
                areviewa.timestart,
                CASE WHEN areviewa.timefinish = 0 THEN null
                     WHEN areviewa.timefinish > areviewa.timestart THEN areviewa.timefinish - areviewa.timestart
                     ELSE 0 END AS duration';
            // To explain that last bit, timefinish can be non-zero and less
            // than timestart when you have two load-balanced servers with very
            // badly synchronised clocks, and a student does a really quick attempt.

        // This part is the same for all cases. Join the users and areview_attempts tables.
        $from = "\n{user} u";
        $from .= "\nLEFT JOIN {areview_attempts} areviewa ON
                                    areviewa.userid = u.id AND areviewa.areview = :areviewid";
        $params = array('areviewid' => $this->areview->id);
        ...
        switch ($this->options->attempts) {
            case areview_attempts_report::ALL_WITH:
                // Show all attempts, including students who are no longer in the course.
                $where = 'areviewa.id IS NOT NULL AND areviewa.preview = 0';
                break;
            case areview_attempts_report::ENROLLED_WITH:
                // Show only students with attempts.
                list($usql, $uparams) = $DB->get_in_or_equal(
                        $reportstudents, SQL_PARAMS_NAMED, 'u');
                $params += $uparams;
                $where = "u.id $usql AND areviewa.preview = 0 AND areviewa.id IS NOT NULL";
                break;
            case areview_attempts_report::ENROLLED_WITHOUT:
                // Show only students without attempts.
                list($usql, $uparams) = $DB->get_in_or_equal(
                        $reportstudents, SQL_PARAMS_NAMED, 'u');
                $params += $uparams;
                $where = "u.id $usql AND areviewa.id IS NULL";
                break;
            case areview_attempts_report::ENROLLED_ALL:
                // Show all students with or without attempts.
                list($usql, $uparams) = $DB->get_in_or_equal(
                        $reportstudents, SQL_PARAMS_NAMED, 'u');
                $params += $uparams;
                $where = "u.id $usql AND (areviewa.preview = 0 OR areviewa.preview IS NULL)";
                break;
        }
        ...
        if ($this->options->states) {
            ...
            $params += $stateparams;
            $where .= " AND (areviewa.state $statesql OR areviewa.state IS NULL)";
        }
    ...
    protected function add_latest_state_join($slot) {
        ...
        $qubaids = new qubaid_join("{areview_attempts} {$alias}areviewa", "{$alias}areviewa.uniqueid",
                "{$alias}areviewa.areview = :{$alias}areviewid", array("{$alias}areviewid" => $this->sql->params['areviewid']));
        ...
        $this->sql->from .= "\nLEFT JOIN $inlineview ON " .
                "$alias.questionusageid = areviewa.uniqueid AND $alias.slot = :{$alias}slot";
    ...
    protected function get_qubaids_condition() {
        ...
        if ($this->is_downloading()) {
            ...
            return new qubaid_join($this->sql->from, 'areviewa.uniqueid',
    ...
    public function get_sort_columns() {
        ...
        $sortcolumns['areviewa.id'] = SORT_ASC;
        return $sortcolumns;
    }
    ...
    public function wrap_html_finish() {
        ...
        echo '<a href="javascript:select_all_in(\'DIV\', null, \'tablecontainer\');">' .
                get_string('selectall', 'areview') . '</a> / ';
        echo '<a href="javascript:deselect_all_in(\'DIV\', null, \'tablecontainer\');">' .
                get_string('selectnone', 'areview') . '</a> ';
        echo '&nbsp;&nbsp;';
        $this->submit_buttons();
        echo '</div>';
    ...
    protected function submit_buttons() {
        global $PAGE;
        if (has_capability('mod/areview:deleteattempts', $this->context)) {
            echo '<input type="submit" id="deleteattemptsbutton" name="delete" value="' .
                    get_string('deleteselected', 'areview_overview') . '"/>';
            $PAGE->requires->event_handler('#deleteattemptsbutton', 'click', 'M.util.show_confirm_dialog',
                    array('message' => get_string('deleteattemptcheck', 'areview')));
        }
    }
}
