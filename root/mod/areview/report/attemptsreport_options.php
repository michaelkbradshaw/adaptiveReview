...
/**
 * Base class for the options that control what is visible in an {@link areview_attempts_report}.
 *
 * @package   mod_areview
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_areview_attempts_report_options {

    /** @var string the report mode. */
    public $mode;

    /** @var object the settings for the areview being reported on. */
    public $areview;

    /** @var object the course module objects for the areview being reported on. */
    public $cm;

    /** @var object the course settings for the course the areview is in. */
    public $course;

    /**
     * @var array form field name => corresponding areview_attempt:: state constant.
     */
    protected static $statefields = array(
        'stateinprogress' => areview_attempt::IN_PROGRESS,
        'stateoverdue'    => areview_attempt::OVERDUE,
        'statefinished'   => areview_attempt::FINISHED,
        'stateabandoned'  => areview_attempt::ABANDONED,
    );

    /**
     * @var string areview_attempts_report::ALL_WITH or areview_attempts_report::ENROLLED_WITH
     *      areview_attempts_report::ENROLLED_WITHOUT or areview_attempts_report::ENROLLED_ALL
     */
    public $attempts = areview_attempts_report::ENROLLED_WITH;

    /** @var int the currently selected group. 0 if no group is selected. */
    public $group = 0;

    /**
     * @var array|null of areview_attempt::IN_PROGRESS, etc. constants. null means
     *      no restriction.
     */
    public $states = array(areview_attempt::IN_PROGRESS, areview_attempt::OVERDUE,
            areview_attempt::FINISHED, areview_attempt::ABANDONED);
    ...
    public $pagesize = areview_attempts_report::DEFAULT_PAGE_SIZE;
     ...
     * @param object $areview the settings for the areview being reported on.
     * @param object $cm the course module objects for the areview being reported on.
     * @param object $coures the course settings for the coures this areview is in.
     */
    public function __construct($mode, $areview, $cm, $course) {
        $this->mode   = $mode;
        $this->areview   = $areview;
        $this->cm     = $cm;
        $this->course = $course;

        $this->usercanseegrades = areview_report_should_show_grades($areview, context_module::instance($cm->id));
    }
    ...
    public function get_url() {
        return new moodle_url('/mod/areview/report.php', $this->get_url_params());
    }
    ...
    public function setup_from_user_preferences() {
        $this->pagesize = get_user_preferences('areview_report_pagesize', $this->pagesize);
    }
    ...
    public function update_user_preferences() {
        set_user_preference('areview_report_pagesize', $this->pagesize);
    }
    ...
    public function resolve_dependencies() {
        if ($this->group) {
            ...
            if ($this->attempts === null || $this->attempts == areview_attempts_report::ALL_WITH) {
                $this->attempts = areview_attempts_report::ENROLLED_WITH;
            }

        } else if (!$this->group && $this->course->id == SITEID) {
            // Force report on front page to show all, unless a group is selected.
            $this->attempts = areview_attempts_report::ALL_WITH;

        } else if (!in_array($this->attempts, array(areview_attempts_report::ALL_WITH, areview_attempts_report::ENROLLED_WITH,
                areview_attempts_report::ENROLLED_WITHOUT, areview_attempts_report::ENROLLED_ALL))) {
            $this->attempts = areview_attempts_report::ENROLLED_WITH;
        }
        ...
        if (!areview_report_can_filter_only_graded($this->areview)) {
        ...
        if ($this->attempts == areview_attempts_report::ENROLLED_WITHOUT) {
            $this->states = null;
            $this->onlygraded = false;
        }

        if ($this->onlygraded) {
            $this->states = array(areview_attempt::FINISHED);
        }

        if ($this->pagesize < 1) {
            $this->pagesize = areview_attempts_report::DEFAULT_PAGE_SIZE;
        }
    }
}
