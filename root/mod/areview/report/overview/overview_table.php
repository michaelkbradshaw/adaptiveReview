...
/**
 * This file defines the areview grades table.
 *
...
require_once($CFG->dirroot . '/mod/areview/report/attemptsreport_table.php');


/**
 * This is a table subclass for displaying the areview grades report.
 *
 * @copyright 2008 Jamie Pratt
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class areview_overview_table extends areview_attempts_report_table {
     ...
     * @param areview_overview_options $options
     * @param array $groupstudents
     * @param array $students
     * @param array $questions
     * @param moodle_url $reporturl
     */
    public function __construct($areview, $context, $qmsubselect,
            areview_overview_options $options, $groupstudents, $students, $questions, $reporturl) {
        parent::__construct('mod-areview-report-overview-report', $areview , $context,
    ...
    protected function add_average_row($label, $users) {
        ...
        $record = $DB->get_record_sql("
                SELECT AVG(areviewa.sumgrades) AS grade, COUNT(areviewa.sumgrades) AS numaveraged
                  FROM $from
                 WHERE $where", $params);
        $record->grade = areview_rescale_grade($record->grade, $this->areview, false);
        ...
        $averagerow = array(
            ...
            'feedbacktext'=> strip_tags(areview_report_feedback_for_grade(
                                        $record->grade, $this->areview->id, $this->context))
        );

        if ($this->options->slotmarks) {
            $dm = new question_engine_data_mapper();
            $qubaids = new qubaid_join($from, 'areviewa.uniqueid', $where, $params);
    ...
    protected function format_average_grade_for_questions($gradeaverages) {
        ...
        foreach ($this->questions as $question) {
            if (isset($gradeaverages[$question->slot]) && $question->maxmark > 0) {
                ...
                $record->grade = areview_rescale_grade(
                        $record->averagefraction * $question->maxmark, $this->areview, false);
    ...
    protected function format_average($record, $question = false) {
        if (is_null($record->grade)) {
            ...
            $average = areview_format_question_grade($this->areview, $record->grade);
        } else {
            $average = areview_format_grade($this->areview, $record->grade);
        }
    ...
    protected function submit_buttons() {
        if (has_capability('mod/areview:regrade', $this->context)) {
            echo '<input type="submit" name="regrade" value="' .
                    get_string('regradeselected', 'areview_overview') . '"/>';
        }
    ...
    public function col_sumgrades($attempt) {
        ...
        $grade = areview_rescale_grade($attempt->sumgrades, $this->areview);
        if ($this->is_downloading()) {
            return $grade;
        }
        ...
        if (isset($this->regradedqs[$attempt->usageid])) {
            ...
            $newsumgrade = areview_rescale_grade($newsumgrade, $this->areview);
            $oldsumgrade = areview_rescale_grade($oldsumgrade, $this->areview);
            $grade = html_writer::tag('del', $oldsumgrade) . '/' .
                    html_writer::empty_tag('br') . $newsumgrade;
        }
        return html_writer::link(new moodle_url('/mod/areview/review.php',
                array('attempt' => $attempt->attempt)), $grade,
                array('title' => get_string('reviewattempt', 'areview')));
    }

    /**
     * @param string $colname the name of the column.
     * @param object $attempt the row of data - see the SQL in display() in
     * mod/areview/report/overview/report.php to see what fields are present,
    ...
    public function other_cols($colname, $attempt) {
        ...
        if ($question->maxmark == 0) {
            ...
            $grade = areview_rescale_grade(
                    $stepdata->fraction * $question->maxmark, $this->areview, 'question');
        }
        ...
        if (isset($this->regradedqs[$attempt->usageid][$slot])) {
            $gradefromdb = $grade;
            $newgrade = areview_rescale_grade(
                    $this->regradedqs[$attempt->usageid][$slot]->newfraction * $question->maxmark,
                    $this->areview, 'question');
            $oldgrade = areview_rescale_grade(
    ...
    public function col_regraded($attempt) {
        if ($attempt->regraded == '') {
            ...
            return get_string('needed', 'areview_overview');
        } else if ($attempt->regraded == 1) {
            return get_string('done', 'areview_overview');
        }
    }
    ...
    public function query_db($pagesize, $useinitialsbar = true) {
        ...
        if ($this->options->slotmarks && has_capability('mod/areview:regrade', $this->context)) {
    ...
    protected function get_regraded_questions() {
        ...
        $regradedqs = $DB->get_records_select('areview_overview_regrades',
                'questionusageid ' . $qubaids->usage_id_in(), $qubaids->usage_id_in_params());
        return areview_report_index_by_keys($regradedqs, array('questionusageid', 'slot'));
    }
}
