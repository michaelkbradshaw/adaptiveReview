...
/**
 * This file renders the areview overview graph.
 *
...
$areviewid = required_param('id', PARAM_INT);
$groupid = optional_param('groupid', 0, PARAM_INT);

$areview = $DB->get_record('areview', array('id' => $areviewid));
$course = $DB->get_record('course', array('id' => $areview->course));
$cm = get_coursemodule_from_instance('areview', $areviewid);

require_login($course, false, $cm);
$modcontext = context_module::instance($cm->id);
require_capability('mod/areview:viewreports', $modcontext);
...
// Pick a sensible number of bands depending on areview maximum grade.
$bands = $areview->grade;
...
$bandwidth = $areview->grade / $bands;
$bandlabels = array();
for ($i = 1; $i <= $bands; $i++) {
    $bandlabels[] = areview_format_grade($areview, ($i - 1) * $bandwidth) . ' - ' .
            areview_format_grade($areview, $i * $bandwidth);
}
...
$line->y_data['allusers'] = areview_report_grade_bands($bandwidth, $bands, $areviewid, $groupusers);
