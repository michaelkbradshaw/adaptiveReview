...
/**
 * Base class for areview reports that are basically a table with one row for each attempt.
 *
 * @copyright 2010 The Open University
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
abstract class areview_attempts_report extends areview_default_report {
    ...
    /** @var object the areview context. */
    protected $context;

    /** @var mod_areview_attempts_report_form The settings form to use. */
    protected $form;
     ...
     * @param object $course
     */
    protected function init($mode, $formclass, $areview, $cm, $course) {
        $this->mode = $mode;
        ...
        $this->qmsubselect = areview_report_qm_filter_select($areview);

        $this->form = new $formclass($this->get_base_url(),
                array('qmsubselect' => $this->qmsubselect, 'areview' => $areview,
    ...
    protected function add_state_column(&$columns, &$headers) {
        $columns[] = 'state';
        $headers[] = get_string('attemptstate', 'areview');
    }
    ...
    protected function add_time_columns(&$columns, &$headers) {
        $columns[] = 'timestart';
        $headers[] = get_string('startedon', 'areview');

        $columns[] = 'timefinish';
        $headers[] = get_string('timecompleted', 'areview');

        $columns[] = 'duration';
        $headers[] = get_string('attemptduration', 'areview');
    }
     ...
     * @param bool $usercanseegrades whether the user is allowed to see grades for this areview.
     * @param array $columns the list of columns. Added to.
     * @param array $headers the columns headings. Added to.
     * @param bool $includefeedback whether to include the feedbacktext columns
     */
    protected function add_grade_columns($areview, $usercanseegrades, &$columns, &$headers, $includefeedback = true) {
        if ($usercanseegrades) {
            $columns[] = 'sumgrades';
            $headers[] = get_string('grade', 'areview') . '/' .
                    areview_format_grade($areview, $areview->grade);
        }

        if ($includefeedback && areview_has_feedback($areview)) {
            $columns[] = 'feedbacktext';
            $headers[] = get_string('feedback', 'areview');
        }
    }
     ...
     * @param mod_areview_attempts_report_options $options the display options.
    ...
    protected function set_up_table_columns($table, $columns, $headers, $reporturl,
            mod_areview_attempts_report_options $options, $collapsible) {
        ...
        if (empty($currentgroup) || $groupstudents) {
            if (optional_param('delete', 0, PARAM_BOOL) && confirm_sesskey()) {
                if ($attemptids = optional_param_array('attemptid', array(), PARAM_INT)) {
                    require_capability('mod/areview:deleteattempts', $this->context);
                    $this->delete_selected_attempts($areview, $cm, $attemptids, $allowed);
    ...
    /**
     * Delete the areview attempts
     * @param object $areview the areview settings. Attempts that don't belong to
     * this areview are not deleted.
     ...
     */
    protected function delete_selected_attempts($areview, $cm, $attemptids, $allowed) {
        global $DB;

        foreach ($attemptids as $attemptid) {
            $attempt = $DB->get_record('areview_attempts', array('id' => $attemptid));
            if (!$attempt || $attempt->areview != $areview->id || $attempt->preview != 0) {
                // Ensure the attempt exists, and belongs to this areview. If not skip.
                continue;
            }
            ...
            add_to_log($areview->course, 'areview', 'delete attempt', 'report.php?id=' . $cm->id,
                    $attemptid, $cm->id);
