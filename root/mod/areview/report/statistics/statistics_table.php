...
/**
 * Quiz statistics report, table for showing statistics of each question in the areview.
 *
...
/**
 * This table has one row for each question in the areview, with sub-rows when
 * random questions appear. There are columns for the various statistics.
 *
 * @copyright 2008 Jamie Pratt
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class areview_statistics_table extends flexible_table {
    /** @var object the areview settings. */
    protected $areview;

    /** @var integer the areview course_module id. */
    protected $cmid;
    ...
    public function __construct() {
        parent::__construct('mod-areview-report-statistics-report');
    }
     ...
     * @param int $cmid the areview course_module id
     * @param moodle_url $reporturl the URL to redisplay this report.
     * @param int $s number of attempts included in the statistics.
     */
    public function statistics_setup($areview, $cmid, $reporturl, $s) {
        ...
        $columns[] = 'number';
        $headers[] = get_string('questionnumber', 'areview_statistics');
        ...
        if (!$this->is_downloading()) {
            ...
            $columns[] = 'qtype';
            $headers[] = get_string('questiontype', 'areview_statistics');
        }

        $columns[] = 'name';
        $headers[] = get_string('questionname', 'areview');

        $columns[] = 's';
        $headers[] = get_string('attempts', 'areview_statistics');

        if ($s > 1) {
            $columns[] = 'facility';
            $headers[] = get_string('facility', 'areview_statistics');

            $columns[] = 'sd';
            $headers[] = get_string('standarddeviationq', 'areview_statistics');
        }

        $columns[] = 'random_guess_score';
        $headers[] = get_string('random_guess_score', 'areview_statistics');

        $columns[] = 'intended_weight';
        $headers[] = get_string('intended_weight', 'areview_statistics');

        $columns[] = 'effective_weight';
        $headers[] = get_string('effective_weight', 'areview_statistics');

        $columns[] = 'discrimination_index';
        $headers[] = get_string('discrimination_index', 'areview_statistics');

        $columns[] = 'discriminative_efficiency';
        $headers[] = get_string('discriminative_efficiency', 'areview_statistics');
    ...
    protected function col_actions($question) {
        return areview_question_action_icons($this->areview, $this->cmid, $question, $this->baseurl);
    }
    ...
    protected function col_name($question) {
        ...
        if ($url) {
            $name = html_writer::link($url, $name,
                    array('title' => get_string('detailedanalysis', 'areview_statistics')));
        }
     ...
     * of maximum mark for the areview. That is, the indended influence this question
     * on the student's overall mark.
     * @param object $question containst the data to display.
     * @return string contents of this table cell.
     */
    protected function col_intended_weight($question) {
        return areview_report_scale_summarks_as_percentage(
                $question->_stats->maxmark, $this->areview);
    }
    ...
    protected function col_effective_weight($question) {
        ...
        if ($question->_stats->negcovar) {
            $negcovar = get_string('negcovar', 'areview_statistics');

            if (!$this->is_downloading()) {
                $negcovar = html_writer::tag('div',
                        $negcovar . $OUTPUT->help_icon('negcovar', 'areview_statistics'),
     ...
     * other questions in this areview.
     * @param object $question containst the data to display.
     * @return string contents of this table cell.
     */
