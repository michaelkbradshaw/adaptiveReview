 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class areview_statistics_response_analyser {
    ...
    /**
     * Store the computed response analysis in the areview_quest_response_stats
     * table.
     * @param int $areviewstatisticsid the cached areview statistics to load the
     * data corresponding to.
     * @return bool true if cached data was found in the database and loaded,
     * otherwise false, to mean no data was loaded.
     */
    public function load_cached($areviewstatisticsid) {
        global $DB;

        $rows = $DB->get_records('areview_quest_response_stats',
                array('areviewstatisticsid' => $areviewstatisticsid,
     ...
     * Store the computed response analysis in the areview_quest_response_stats
     * table.
     * @param int $areviewstatisticsid the cached areview statistics this correspons to.
     */
    public function store_cached($areviewstatisticsid) {
        global $DB;
        ...
        foreach ($this->responses as $subpartid => $partdata) {
            foreach ($partdata as $responseclassid => $classdata) {
                foreach ($classdata as $response => $data) {
                    $row = new stdClass();
                    $row->areviewstatisticsid = $areviewstatisticsid;
                    ...
                    $DB->insert_record('areview_quest_response_stats', $row, false);
                }
            }
        }
    }
