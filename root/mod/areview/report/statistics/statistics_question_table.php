 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class areview_statistics_question_table extends flexible_table {
    ...
    public function __construct($qid) {
        parent::__construct('mod-areview-report-statistics-question-table' . $qid);
    }
    ...
    public function question_setup($reporturl, $questiondata,
            areview_statistics_response_analyser $responesstats) {
        $this->questiondata = $questiondata;

        $this->define_baseurl($reporturl->out());
        $this->collapsible(false);
        $this->set_attribute('class', 'generaltable generalbox boxaligncenter areviewresponseanalysis');
        ...
        if ($responesstats->has_subparts()) {
            $columns[] = 'part';
            $headers[] = get_string('partofquestion', 'areview_statistics');
        }

        if ($responesstats->has_response_classes()) {
            $columns[] = 'responseclass';
            $headers[] = get_string('modelresponse', 'areview_statistics');

            if ($responesstats->has_actual_responses()) {
                $columns[] = 'response';
                $headers[] = get_string('actualresponse', 'areview_statistics');
            }

        } else {
            $columns[] = 'response';
            $headers[] = get_string('response', 'areview_statistics');
        }

        $columns[] = 'fraction';
        $headers[] = get_string('optiongrade', 'areview_statistics');

        $columns[] = 'count';
        $headers[] = get_string('count', 'areview_statistics');

        $columns[] = 'frequency';
        $headers[] = get_string('frequency', 'areview_statistics');
