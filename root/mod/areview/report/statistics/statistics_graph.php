...
/**
 * This script renders the areview statistics graph.
 *
 * It takes one parameter, the areview_statistics.id. This is enough to identify the
 * areview etc.
 *
...
// Get the parameters.
$areviewstatisticsid = required_param('id', PARAM_INT);

// Load enough data to check permissions.
$areviewstatistics = $DB->get_record('areview_statistics', array('id' => $areviewstatisticsid));
$areview = $DB->get_record('areview', array('id' => $areviewstatistics->areviewid), '*', MUST_EXIST);
$cm = get_coursemodule_from_instance('areview', $areview->id);

// Check access.
require_login($areview->course, false, $cm);
$modcontext = context_module::instance($cm->id);
require_capability('areview/statistics:view', $modcontext);
...
if ($areviewstatistics->groupid && !in_array($areviewstatistics->groupid, array_keys($groups))) {
...
$questionstatistics = $DB->get_records_select('areview_question_statistics',
        'areviewstatisticsid = ? AND slot IS NOT NULL', array($areviewstatistics->id));
...
$graph->parameter['x_label'] = get_string('position', 'areview_statistics');
...
$fieldstoplot = array(
    ...
    'discriminativeefficiency' => get_string('discriminative_efficiency', 'areview_statistics')
);
