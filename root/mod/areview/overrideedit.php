...
/**
 * This page handles editing and creation of areview overrides
 *
 * @package    mod
 * @subpackage areview
...
$url = new moodle_url('/mod/areview/overrideedit.php');
if ($action) {
...
// Merge areview defaults with data.
$keys = array('timeopen', 'timeclose', 'timelimit', 'attempts', 'password');
foreach ($keys as $key) {
    if (!isset($data->{$key}) || $reset) {
        $data->{$key} = $areview->{$key};
    }
}
...
$overridelisturl = new moodle_url('/mod/areview/overrides.php', array('cmid'=>$cm->id));
if (!$groupmode) {
    $overridelisturl->param('mode', 'user');
}

// Setup the form.
$mform = new areview_override_form($url, $cm, $areview, $context, $groupmode, $override);
$mform->set_data($data);
...
if ($mform->is_cancelled()) {
    ...
    // Process the data.
    $fromform->areview = $areview->id;

    // Replace unchanged values with null.
    foreach ($keys as $key) {
        if ($fromform->{$key} == $areview->{$key}) {
            $fromform->{$key} = null;
        }
    }
    ...
    if ($userorgroupchanged) {
        ...
        if ($oldoverride = $DB->get_record('areview_overrides', $conditions)) {
            ...
            $DB->delete_records('areview_overrides', array('id' => $oldoverride->id));
        }
    }

    if (!empty($override->id)) {
        $fromform->id = $override->id;
        $DB->update_record('areview_overrides', $fromform);
    } else {
        unset($fromform->id);
        $fromform->id = $DB->insert_record('areview_overrides', $fromform);
    }

    areview_update_open_attempts(array('areviewid'=>$areview->id));
    areview_update_events($areview, $fromform);

    add_to_log($cm->course, 'areview', 'edit override',
            "overrideedit.php?id=$fromform->id", $areview->id, $cm->id);
...
// Print the form.
$pagetitle = get_string('editoverride', 'areview');
