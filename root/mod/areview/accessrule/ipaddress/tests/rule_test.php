...
/**
 * Unit tests for the areviewaccess_ipaddress plugin.
 *
...
global $CFG;
require_once($CFG->dirroot . '/mod/areview/accessrule/ipaddress/rule.php');
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class areviewaccess_ipaddress_testcase extends basic_testcase {
    public function test_ipaddress_access_rule() {
        ...
        $areview->subnet = getremoteaddr(null);
        if (!empty($areview->subnet)) {
            $areview->questions = '';
            $areviewobj = new areview($areview, $cm, null);
            $rule = new areviewaccess_ipaddress($areviewobj, 0);
