...
/**
 * Unit tests for the areviewaccess_timelimit plugin.
 *
...
global $CFG;
require_once($CFG->dirroot . '/mod/areview/accessrule/timelimit/rule.php');
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class areviewaccess_timelimit_testcase extends basic_testcase {
    public function test_time_limit_access_rule() {
        $areview = new stdClass();
        $areview->timelimit = 3600;
        $areview->questions = '';
        $cm = new stdClass();
        $cm->id = 0;
        $areviewobj = new areview($areview, $cm, null);
        $rule = new areviewaccess_timelimit($areviewobj, 10000);
        $attempt = new stdClass();

        $this->assertEquals($rule->description(),
            get_string('areviewtimelimit', 'areviewaccess_timelimit', format_time(3600)));
