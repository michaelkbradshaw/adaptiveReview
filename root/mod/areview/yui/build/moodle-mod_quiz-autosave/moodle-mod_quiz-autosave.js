YUI.add('moodle-mod_areview-autosave', function (Y, NAME) {
...
/**
 * Auto-save functionality for during areview attempts.
 *
 * @package   mod_areview
...
M.mod_areview.autosave = {
    ...
    AUTOSAVE_HANDLER: M.cfg.wwwroot + '/mod/areview/autosave.ajax.php',
     ...
     */
    init: function(delay) {
        this.form = Y.one(this.SELECTORS.areview_FORM);
        if (!this.form) {
            return;
        }
    ...
    is_time_nearly_over: function() {
        return M.mod_areview.timer && M.mod_areview.timer.endtime &&
                (new Date().getTime() + 2*this.delay) > M.mod_areview.timer.endtime;
    },
