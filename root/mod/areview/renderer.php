...
/**
 * Defines the renderer for the areview module.
 *
 * @package    mod
 * @subpackage areview
...
/**
 * The renderer for the areview module.
 *
 * @copyright  2011 The Open University
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_areview_renderer extends plugin_renderer_base {
    /**
     * Builds the review page
     *
     * @param areview_attempt $attemptobj an instance of areview_attempt.
     ...
     * @param mod_areview_display_options $displayoptions instance of mod_areview_display_options.
     * @param array $summarydata contains all table data
     * @return $output containing html data.
     */
    public function review_page(areview_attempt $attemptobj, $slots, $page, $showall,
                                $lastpage, mod_areview_display_options $displayoptions,
                                $summarydata) {
     ...
     * @return $output containing html data.
     */
    public function review_question_page(areview_attempt $attemptobj, $slot, $seq,
            mod_areview_display_options $displayoptions, $summarydata) {
    ...
    public function review_summary_table($summarydata, $page) {
        ...
        $output .= html_writer::start_tag('table', array(
                'class' => 'generaltable generalbox areviewreviewsummary'));
     ...
     *
     * @param areview_attempt $attemptobj instance of areview_attempt
     ...
     * @param mod_areview_display_options $displayoptions instance of mod_areview_display_options
     */
    public function questions(areview_attempt $attemptobj, $reviewing, $slots, $page, $showall,
                              mod_areview_display_options $displayoptions) {
        $output = '';
    ...
    public function review_form($page, $showall, $displayoptions, $content, $attemptobj) {
        ...
        $this->page->requires->js_init_call('M.mod_areview.init_review_form', null, false,
                areview_get_js_module());
     ...
     * @param areview_attempt $attemptobj instance of areview_attempt
     */
    public function finish_review_link(areview_attempt $attemptobj) {
        $url = $attemptobj->view_url();

        if ($attemptobj->get_access_manager(time())->attempt_must_be_in_popup()) {
            $this->page->requires->js_init_call('M.mod_areview.secure_window.init_close_button',
                    array($url), areview_get_js_module());
            return html_writer::empty_tag('input', array('type' => 'button',
                    'value' => get_string('finishreview', 'areview'),
                    'id' => 'secureclosebutton'));

        } else {
            return html_writer::link($url, get_string('finishreview', 'areview'));
        }
    }
     ...
     */
    public function review_next_navigation(areview_attempt $attemptobj, $page, $lastpage) {
        if ($lastpage) {
    ...
    /**
     * Return the HTML of the areview timer.
     * @return string HTML content.
     */
    public function countdown_timer(areview_attempt $attemptobj, $timenow) {
        ...
        if ($timeleft !== false) {
            ...
            if (!$ispreview) {
                ...
                // this will just have the effect of causing the areview to be submitted immediately.
                $timerstartvalue = max($timerstartvalue, 1);
            }
            $this->initialise_timer($timerstartvalue, $ispreview);
        }

        return html_writer::tag('div', get_string('timeleft', 'areview') . ' ' .
                html_writer::tag('span', '', array('id' => 'areview-time-left')),
                array('id' => 'areview-timer', 'role' => 'timer',
    ...
    public function restart_preview_button($url) {
        return $this->single_button($url, get_string('startnewpreview', 'areview'));
    }

    /**
     * Outputs the navigation block panel
     *
     * @param areview_nav_panel_base $panel instance of areview_nav_panel_base
     */
    public function navigation_panel(areview_nav_panel_base $panel) {
        ...
        $this->page->requires->js_init_call('M.mod_areview.nav.init', null, false,
                areview_get_js_module());
    ...
    /**
     * Returns the areviewzes navigation button
     *
     * @param areview_nav_question_button $button
     */
    protected function render_areview_nav_question_button(areview_nav_question_button $button) {
        $classes = array('qnbutton', $button->stateclass, $button->navmethod);
        $attributes = array();

        if ($button->currentpage) {
            $classes[] = 'thispage';
            $attributes[] = get_string('onthispage', 'areview');
        }
        ...
        $tagcontents = html_writer::tag('span', '', array('class' => 'thispageholder')) .
                        ...
                        get_string($qnostring, 'areview', $a);
     ...
     *
     * @param mod_areview_links_to_other_attempts $links
     */
    protected function render_mod_areview_links_to_other_attempts(
            mod_areview_links_to_other_attempts $links) {
        $attemptlinks = array();
    ...
    public function start_attempt_page(areview $areviewobj, mod_areview_preflight_check_form $mform) {
        $output = '';
        $output .= $this->header();
        $output .= $this->areview_intro($areviewobj->get_areview(), $areviewobj->get_cm());
        ob_start();
        $mform->display();
        $output .= ob_get_clean();
        $output .= $this->footer();
     ...
     * @param areview_attempt $attemptobj Instance of areview_attempt
     * @param int $page Current page number
     * @param areview_access_manager $accessmanager Instance of areview_access_manager
    ...
    public function attempt_page($attemptobj, $page, $accessmanager, $messages, $slots, $id,
        ...
        $output = '';
        $output .= $this->header();
        $output .= $this->areview_notices($messages);
     ...
     */
    public function areview_notices($messages) {
        if (!$messages) {
            return '';
        }
        return $this->box($this->heading(get_string('accessnoticesheader', 'areview'), 3) .
                $this->access_messages($messages), 'areviewaccessnotices');
    }

    /**
     * Ouputs the form for making an attempt
     *
     * @param areview_attempt $attemptobj
     * @param int $page Current page number
    ...
    public function initialise_timer($timerstartvalue, $ispreview) {
        ...
        $this->page->requires->js_init_call('M.mod_areview.timer.init', $options, false, areview_get_js_module());
    }
    ...
    public function close_attempt_popup($url, $message = '') {
        ...
        if ($message) {
            ...
            $output .= html_writer::tag('p', get_string('windowclosing', 'areview'));
            $delay = 5;
        } else {
            $output .= html_writer::tag('p', get_string('pleaseclose', 'areview'));
            $delay = 0;
        }
        $this->page->requires->js_init_call('M.mod_areview.secure_window.close',
                array($url, $delay), false, areview_get_js_module());
     ...
     *
     * @param areview_attempt $attemptobj
     * @param mod_areview_display_options $displayoptions
     */
    public function summary_page($attemptobj, $displayoptions) {
        $output = '';
        $output .= $this->header();
        $output .= $this->heading(format_string($attemptobj->get_areview_name()));
        $output .= $this->heading(get_string('summaryofattempt', 'areview'), 3);
    ...
    public function summary_table($attemptobj, $displayoptions) {
        ...
        $table->attributes['class'] = 'generaltable areviewsummaryofattempt boxaligncenter';
        $table->head = array(get_string('question', 'areview'), get_string('status', 'areview'));
        $table->align = array('left', 'left');
        $table->size = array('', '');
        $markscolumn = $displayoptions->marks >= question_display_options::MARK_AND_MAX;
        if ($markscolumn) {
            $table->head[] = get_string('marks', 'areview');
            $table->align[] = 'left';
            $table->size[] = '';
        }
        $table->data = array();
    ...
    public function summary_page_controls($attemptobj) {
        ...
        if ($attemptobj->get_state() == areview_attempt::IN_PROGRESS) {
            $button = new single_button(
                    new moodle_url($attemptobj->attempt_url(null, $attemptobj->get_currentpage())),
                    get_string('returnattempt', 'areview'));
        ...
        $button = new single_button(
                ...
                get_string('submitallandfinish', 'areview'));
        $button->id = 'responseform';
        if ($attemptobj->get_state() == areview_attempt::IN_PROGRESS) {
            $button->add_action(new confirm_action(get_string('confirmclose', 'areview'), null,
                    get_string('submitallandfinish', 'areview')));
        }

        $duedate = $attemptobj->get_due_date();
        $message = '';
        if ($attemptobj->get_state() == areview_attempt::OVERDUE) {
            $message = get_string('overduemustbesubmittedby', 'areview', userdate($duedate));

        } else if ($duedate) {
            $message = get_string('mustbesubmittedby', 'areview', userdate($duedate));
        }
     ...
     * @param array $areview Array conting areview data
     * @param int $cm Course Module ID
     * @param int $context The page context ID
     * @param array $infomessages information about this areview
     * @param mod_areview_view_object $viewobj
     ...
     *      attempt this areview now, if appicable this areview
     */
    public function view_page($course, $areview, $cm, $context, $viewobj) {
        $output = '';
        $output .= $this->view_information($areview, $cm, $context, $viewobj->infomessages);
        $output .= $this->view_table($areview, $context, $viewobj);
        $output .= $this->view_best_score($viewobj);
        $output .= $this->view_result_info($areview, $context, $cm, $viewobj);
        $output .= $this->box($this->view_page_buttons($viewobj), 'areviewattempt');
        return $output;
    }
     ...
     * @param mod_areview_view_object $viewobj the information required to display
     * the view page.
     * @return string HTML to output.
     */
    public function view_page_buttons(mod_areview_view_object $viewobj) {
        $output = '';
        ...
        if ($viewobj->buttontext) {
            ...
            $output .= $this->single_button($viewobj->backtocourseurl,
                    get_string('backtocourse', 'areview'), 'get',
     ...
     * @param array $areview Array containging areview date
    ...
    public function start_attempt_button($buttontext, moodle_url $url,
        ...
        $button->class .= ' areviewstartbuttondiv';

        $warning = '';
        if ($popuprequired) {
            $this->page->requires->js_module(areview_get_js_module());
            $this->page->requires->js('/mod/areview/module.js');
            $popupaction = new popup_action('click', $url, 'areviewpopup', $popupoptions);

            $button->class .= ' areviewsecuremoderequired';
            $button->add_action(new component_action('click',
                    'M.mod_areview.secure_window.start_attempt_action', array(
                        'url' => $url->out(false),
                        'windowname' => 'areviewpopup',
                        'options' => $popupaction->get_js_options(),
                        'fullscreen' => true,
                        'startattemptwarning' => $startattemptwarning,
                    )));

            $warning = html_writer::tag('noscript', $this->heading(get_string('noscript', 'areview')));

        } else if ($startattemptwarning) {
            $button->add_action(new confirm_action($startattemptwarning, null,
                    get_string('startattempt', 'areview')));
        }
    ...
    public function no_questions_message($canedit, $editurl) {
        $output = '';
        $output .= $this->notification(get_string('noquestions', 'areview'));
        if ($canedit) {
            $output .= $this->single_button($editurl, get_string('editareview', 'areview'), 'get');
        }
    ...
    /**
     * Outputs an error message for any guests accessing the areview
     *
     * @param int $course The course ID
     * @param array $areview Array contingin areview data
     * @param int $cm Course Module ID
     * @param int $context The page contect ID
     * @param array $messages Array containing any messages
     */
    public function view_page_guest($course, $areview, $cm, $context, $messages) {
        $output = '';
        $output .= $this->view_information($areview, $cm, $context, $messages);
        $guestno = html_writer::tag('p', get_string('guestsno', 'areview'));
     ...
     * @param array $messages Array containing any messages
     */
    public function view_page_notenrolled($course, $areview, $cm, $context, $messages) {
        global $CFG;
        $output = '';
        $output .= $this->view_information($areview, $cm, $context, $messages);
        $youneedtoenrol = html_writer::tag('p', get_string('youneedtoenrol', 'areview'));
     ...
     * @return string HTML to output.
     */
    public function view_information($areview, $cm, $context, $messages) {
        global $CFG;

        $output = '';

        // Print areview name and description.
        $output .= $this->heading(format_string($areview->name));
        $output .= $this->areview_intro($areview, $cm);

        // Output any access messages.
        if ($messages) {
            $output .= $this->box($this->access_messages($messages), 'areviewinfo');
        }

        // Show number of attempts summary to those who can view reports.
        if (has_capability('mod/areview:viewreports', $context)) {
            if ($strattemptnum = $this->areview_attempt_summary_link_to_reports($areview, $cm,
                    $context)) {
                $output .= html_writer::tag('div', $strattemptnum,
                        array('class' => 'areviewattemptcounts'));
            }
        }
        return $output;
    }
    ...
    /**
     * Output the areview intro.
     * @param object $areview the areview settings.
     * @param object $cm the course_module object.
     * @return string HTML to output.
     */
    public function areview_intro($areview, $cm) {
        if (html_is_blank($areview->intro)) {
            return '';
        }

        return $this->box(format_module_intro('areview', $areview, $cm->id), 'generalbox', 'intro');
    }
    ...
    public function view_table_heading() {
        return $this->heading(get_string('summaryofattempts', 'areview'));
    }

    /**
     * Generates the table of data
     *
     * @param array $areview Array contining areview data
     * @param int $context The page context ID
     * @param mod_areview_view_object $viewobj
     */
    public function view_table($areview, $context, $viewobj) {
        if (!$viewobj->attempts) {
            return '';
        }
        ...
        $table = new html_table();
        $table->attributes['class'] = 'generaltable areviewattemptsummary';
        $table->head = array();
        $table->align = array();
        $table->size = array();
        if ($viewobj->attemptcolumn) {
            $table->head[] = get_string('attemptnumber', 'areview');
            $table->align[] = 'center';
            $table->size[] = '';
        }
        $table->head[] = get_string('attemptstate', 'areview');
        $table->align[] = 'left';
        $table->size[] = '';
        ...
        if ($viewobj->gradecolumn) {
            $table->head[] = get_string('grade') . ' / ' .
                    areview_format_grade($areview, $areview->grade);
            $table->align[] = 'center';
            $table->size[] = '';
        }
        if ($viewobj->canreviewmine) {
            $table->head[] = get_string('review', 'areview');
            $table->align[] = 'center';
            $table->size[] = '';
        }
        if ($viewobj->feedbackcolumn) {
            $table->head[] = get_string('feedback', 'areview');
            $table->align[] = 'left';
            $table->size[] = '';
        }
        ...
        foreach ($viewobj->attemptobjs as $attemptobj) {
            ...
            if ($viewobj->attemptcolumn) {
                if ($attemptobj->is_preview()) {
                    $row[] = get_string('preview', 'areview');
                } else {
...
/* //don't show marks!
            if ($viewobj->markcolumn) {
                if ($attemptoptions->marks >= question_display_options::MARK_AND_MAX &&
                	...
                	$row[] = areview_format_grade($areview, $attemptobj->get_sum_marks());
...
*/
  //          $row[] = '';
           ...
           // $attemptgrade = areview_rescale_grade($attemptobj->get_sum_marks(), $areview, false);
            $attemptgrade = areview_rescale_grade($attemptobj->get_sum_marks()/$attemptobj->get_num_questions(), $areview, false);
  ...
  //          $attemptobj->get_num_questions();
            ...
            if ($viewobj->gradecolumn) {
                if ($attemptoptions->marks >= question_display_options::MARK_AND_MAX &&
                    ...
                    if ($viewobj->overallstats && !$attemptobj->is_preview()
                            ...
                            && $areview->grademethod == areview_GRADEHIGHEST) {
                        $table->rowclasses[$attemptobj->get_attempt_number()] = 'bestrow';
                    }

                    $row[] = areview_format_grade($areview, $attemptgrade);
                } else {
                    $row[] = '';
                }
            }
            ...
            if ($viewobj->feedbackcolumn && $attemptobj->is_finished()) {
                if ($attemptoptions->overallfeedback) {
                    $row[] = areview_feedback_for_grade($attemptgrade, $areview, $context);
                } else {
                    $row[] = '';
                }
            }
     ...
     * @param areview_attempt $attemptobj the attempt
    ...
    public function attempt_state($attemptobj) {
        switch ($attemptobj->get_state()) {
            ...
            case areview_attempt::OVERDUE:
                return get_string('stateoverdue', 'areview') . html_writer::tag('span',
                        get_string('stateoverduedetails', 'areview',
                                userdate($attemptobj->get_due_date())),
                        array('class' => 'statedetails'));

            case areview_attempt::FINISHED:
                return get_string('statefinished', 'areview') . html_writer::tag('span',
                        get_string('statefinisheddetails', 'areview',
    ...
    public function view_best_score($viewobj) {
        $output = '';
        // Print information about the student's best score for this areview if possible.
        if (!$viewobj->moreattempts) {
            $output .= $this->heading(get_string('nomoreattempts', 'areview'));
        }
        return $output;
    }

    /**
     * Generates data pertaining to areview results
     *
     * @param array $areview Array containing areview data
     * @param int $context The page context ID
     * @param int $cm The Course Module Id
     * @param mod_areview_view_object $viewobj
     */
    public function view_result_info($areview, $context, $cm, $viewobj) {
        $output = '';
        ...
        if ($viewobj->overallstats) {
            if ($viewobj->moreattempts) {
                $a = new stdClass();
                $a->method = areview_get_grading_option_name($areview->grademethod);
                $a->mygrade = areview_format_grade($areview, $viewobj->mygrade);
                $a->areviewgrade = areview_format_grade($areview, $areview->grade);
                $resultinfo .= $this->heading(get_string('gradesofar', 'areview', $a), 2, 'main');
            } else {
                $a = new stdClass();
                $a->grade = areview_format_grade($areview, $viewobj->mygrade);
                $a->maxgrade = areview_format_grade($areview, $areview->grade);
                $a = get_string('outofshort', 'areview', $a);
                $resultinfo .= $this->heading(get_string('yourfinalgradeis', 'areview', $a), 2,
                        'main');
            }
        }
        ...
        if ($viewobj->gradebookfeedback) {
            $resultinfo .= $this->heading(get_string('comment', 'areview'), 3, 'main');
            $resultinfo .= '<p class="areviewteacherfeedback">'.$viewobj->gradebookfeedback.
                    "</p>\n";
        }
        if ($viewobj->feedbackcolumn) {
            $resultinfo .= $this->heading(get_string('overallfeedback', 'areview'), 3, 'main');
            $resultinfo .= html_writer::tag('p',
                    areview_feedback_for_grade($viewobj->mygrade, $areview, $context),
                    array('class' => 'areviewgradefeedback'))."\n";
        }
    ...
    public function review_link($url, $reviewinpopup, $popupoptions) {
        if ($reviewinpopup) {
            $button = new single_button($url, get_string('review', 'areview'));
            $button->add_action(new popup_action('click', $url, 'areviewpopup', $popupoptions));
            return $this->render($button);

        } else {
            return html_writer::link($url, get_string('review', 'areview'),
                    array('title' => get_string('reviewthisattempt', 'areview')));
        }
    }
     ...
     */
    public function areview_attempt_summary_link_to_reports($areview, $cm, $context,
...
class mod_areview_links_to_other_attempts implements renderable {
    /**
...
class mod_areview_view_object {
    /** @var array $infomessages of messages with information to display about the areview. */
    public $infomessages;
    /** @var array $attempts contains all the user's attempts at this areview. */
    public $attempts;
    /** @var array $attemptobjs areview_attempt objects corresponding to $attempts. */
    public $attemptobjs;
    /** @var areview_access_manager $accessmanager contains various access rules. */
    public $accessmanager;
    /** @var bool $canreviewmine whether the current user has the capability to
     *       review their own attempts. */
    public $canreviewmine;
    /** @var bool $canedit whether the current user has the capability to edit the areview. */
    public $canedit;
    /** @var moodle_url $editurl the URL for editing this areview. */
    public $editurl;
    ...
    public $numattempts;
    /** @var float $mygrade contains the user's final grade for a areview. */
    public $mygrade;
    ...
    public $popupoptions;
    /** @var bool $areviewhasquestions whether the areview has any questions. */
    public $areviewhasquestions;
}
