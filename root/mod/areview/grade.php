...
$cm = get_coursemodule_from_id('areview', $id, 0, false, MUST_EXIST);
$course = $DB->get_record('course', array('id' => $cm->course), '*', MUST_EXIST);
$areview = $DB->get_record('areview', array('id' => $cm->instance), '*', MUST_EXIST);
require_login($course, false, $cm);

$reportlist = areview_report_list(context_module::instance($cm->id));
if (empty($reportlist) || $userid == $USER->id) {
    ...
    // appear in the gradebook are unlikely to be able to see areview reports,
    // but it is possible.)
    redirect(new moodle_url('/mod/areview/view.php', array('id' => $cm->id)));
}
...
if ($userid) {
    ...
    $attempts = areview_get_user_attempts($areview->id, $userid, 'finished');
    $attempt = null;
    ...
    if ($attempt) {
        $attemptobj = new areview_attempt($attempt, $areview, $cm, $course, false);
...
redirect(new moodle_url('/mod/areview/report.php', array(
