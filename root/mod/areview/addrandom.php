...
/**
 * Fallback page of /mod/areview/edit.php add random question dialog,
...
list($thispageurl, $contexts, $cmid, $cm, $areview, $pagevars) =
        question_edit_setup('editq', '/mod/areview/addrandom.php', true);
...
if ($returnurl) {
    ...
    $returnurl = new moodle_url('/mod/areview/edit.php', array('cmid' => $cmid));
}
if ($scrollpos) {
...
$mform = new areview_add_random_form(new moodle_url('/mod/areview/addrandom.php'), $contexts);
...
if ($data = $mform->get_data()) {
    if (!empty($data->existingcategory)) {
        ...
        add_to_log($areview->course, 'areview', 'addcategory',
    ...
    areview_add_random_questions($areview, $addonpage, $categoryid, 1, $includesubcategories);
...
// Setup $PAGE.
$streditingareview = get_string('editinga', 'moodle', get_string('modulename', 'areview'));
$PAGE->navbar->add($streditingareview);
$PAGE->set_title($streditingareview);
$PAGE->set_heading($course->fullname);
echo $OUTPUT->header();

if (!$areviewname = $DB->get_field($cm->modname, 'name', array('id' => $cm->instance))) {
            print_error('invalidcoursemodule');
}

echo $OUTPUT->heading(get_string('addrandomquestiontoareview', 'areview', $areviewname), 2, 'mdl-left');
$mform->display();
echo $OUTPUT->footer();
