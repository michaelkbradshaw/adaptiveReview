...
/**
 * This page handles listing of areview overrides
 *
 * @package    mod_areview
...
if (! $cm = get_coursemodule_from_id('areview', $cmid)) {
...
$url = new moodle_url('/mod/areview/overrides.php', array('cmid'=>$cm->id, 'mode'=>$mode));
...
$PAGE->set_title(get_string('overrides', 'areview'));
$PAGE->set_heading($course->fullname);
echo $OUTPUT->header();

// Delete orphaned group overrides.
$sql = 'SELECT o.id
            FROM {areview_overrides} o LEFT JOIN {groups} g
            ON o.groupid = g.id
...
if (!empty($orphaned)) {
    $DB->delete_records_list('areview_overrides', 'id', array_keys($orphaned));
}
...
if ($groupmode) {
    ...
    $sql = 'SELECT o.*, g.name
                ...
                WHERE o.areview = :areviewid
                ORDER BY g.name';
    $params = array('areviewid' => $areview->id);
} else {
...
$overridedeleteurl = new moodle_url('/mod/areview/overridedelete.php');
$overrideediturl = new moodle_url('/mod/areview/overrideedit.php');
...
foreach ($overrides as $override) {
    ...
    if (!$groupmode) {
        if (!has_capability('mod/areview:attempt', $context, $override->userid)) {
            // User not allowed to take the areview.
            $active = false;
    ...
    if (isset($override->timeopen)) {
        $fields[] = get_string('areviewopens', 'areview');
        $values[] = $override->timeopen > 0 ?
                userdate($override->timeopen) : get_string('noopen', 'areview');
    }

    // Format timeclose.
    if (isset($override->timeclose)) {
        $fields[] = get_string('areviewcloses', 'areview');
        $values[] = $override->timeclose > 0 ?
                userdate($override->timeclose) : get_string('noclose', 'areview');
    }

    // Format timelimit.
    if (isset($override->timelimit)) {
        $fields[] = get_string('timelimit', 'areview');
        $values[] = $override->timelimit > 0 ?
                format_time($override->timelimit) : get_string('none', 'areview');
    }

    // Format number of attempts.
    if (isset($override->attempts)) {
        $fields[] = get_string('attempts', 'areview');
    ...
    if (isset($override->password)) {
        $fields[] = get_string('requirepassword', 'areview');
        $values[] = $override->password !== '' ?
                get_string('enabled', 'areview') : get_string('none', 'areview');
    }
...
echo html_writer::start_tag('div', array('id' => 'areviewoverrides'));
if (count($table->data)) {
    echo html_writer::table($table);
}
if ($hasinactive) {
    echo $OUTPUT->notification(get_string('inactiveoverridehelp', 'areview'), 'dimmed_text');
}
...
if ($groupmode) {
    if (empty($groups)) {
        // There are no groups.
        echo $OUTPUT->notification(get_string('groupsnone', 'areview'), 'error');
        $options['disabled'] = true;
    }
    echo $OUTPUT->single_button($overrideediturl->out(true,
            array('action' => 'addgroup', 'cmid' => $cm->id)),
            get_string('addnewgroupoverride', 'areview'), 'post', $options);
} else {
    $users = array();
    // See if there are any students in the areview.
    if (!empty($CFG->enablegroupmembersonly) && $cm->groupmembersonly) {
        ...
        if (!empty($limitgroups)) {
            $users = get_users_by_capability($context, 'mod/areview:attempt', 'u.id',
    ...
    if (empty($users)) {
        ...
        echo $OUTPUT->notification(get_string('usersnone', 'areview'), 'error');
        $options['disabled'] = true;
    }
    echo $OUTPUT->single_button($overrideediturl->out(true,
            array('action' => 'adduser', 'cmid' => $cm->id)),
            get_string('addnewuseroverride', 'areview'), 'post', $options);
}
