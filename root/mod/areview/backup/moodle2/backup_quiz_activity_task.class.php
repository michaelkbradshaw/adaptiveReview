...
/**
 * Defines backup_areview_activity_task class
 *
 * @package     mod_areview
 * @category    backup
...
require_once($CFG->dirroot . '/mod/areview/backup/moodle2/backup_areview_stepslib.php');
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class backup_areview_activity_task extends backup_activity_task {
    ...
    protected function define_my_steps() {
        // Generate the areview.xml file containing all the areview information
        // and annotating used questions.
        $this->add_step(new backup_areview_activity_structure_step('areview_structure', 'areview.xml'));

        // Note: Following  steps must be present
        // in all the activities using question banks (only areview for now)
    ...
    public static function encode_content_links($content) {
        ...
        // Link to the list of areviewzes.
        $search="/(".$base."\/mod\/areview\/index.php\?id\=)([0-9]+)/";
        $content= preg_replace($search, '$@QUIZINDEX*$2@$', $content);

        // Link to areview view by moduleid.
        $search="/(".$base."\/mod\/areview\/view.php\?id\=)([0-9]+)/";
        $content= preg_replace($search, '$@QUIZVIEWBYID*$2@$', $content);

        // Link to areview view by areviewid.
        $search="/(".$base."\/mod\/areview\/view.php\?q\=)([0-9]+)/";
