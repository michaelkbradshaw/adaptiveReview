 ...
 */
class moodle1_mod_areview_handler extends moodle1_mod_handler {
    ...
    public function get_paths() {
        return array(
            new convert_path(
                'areview', '/MOODLE_BACKUP/COURSE/MODULES/MOD/QUIZ',
                array(
                    'newfields' => array(
                        'showuserpicture'   => 0,
     ...
     * data available
     */
    public function process_areview($data) {
        global $CFG;

        // Replay the upgrade step 2008081501.
        if (is_null($data['sumgrades'])) {
            $data['sumgrades'] = 0;
            // TODO for user data: areview_attempts SET sumgrades=0 WHERE sumgrades IS NULL.
            // TODO for user data: areview_grades.grade should be not be null , convert to default 0.
        }
        ...
        $this->fileman = $this->converter->get_file_manager($contextid, 'mod_areview');
        ...
        // Start writing areview.xml.
        $this->open_xml_writer("activities/areview_{$this->moduleid}/areview.xml");
        $this->xmlwriter->begin_tag('activity', array('id' => $instanceid,
                'moduleid' => $this->moduleid, 'modulename' => 'areview',
                'contextid' => $contextid));
        $this->xmlwriter->begin_tag('areview', array('id' => $instanceid));
    ...
    public function on_areview_question_instances_start() {
        $this->xmlwriter->begin_tag('question_instances');
    }

    public function on_areview_question_instances_end() {
        $this->xmlwriter->end_tag('question_instances');
    }

    public function process_areview_question_instance($data) {
        $this->write_xml('question_instance', $data, array('/question_instance/id'));
    }

    public function on_areview_feedbacks_start() {
        $this->xmlwriter->begin_tag('feedbacks');
    }

    public function on_areview_feedbacks_end() {
        $this->xmlwriter->end_tag('feedbacks');
    }

    public function process_areview_feedback($data) {
    ...
    /**
     * This is executed when we reach the closing </MOD> tag of our 'areview' path
     */
    public function on_areview_end() {

        // Append empty <overrides> subpath element.
        $this->write_xml('overrides', array());

        // Finish writing areview.xml.
        $this->xmlwriter->end_tag('areview');
        $this->xmlwriter->end_tag('activity');
        $this->close_xml_writer();

        // Write inforef.xml.
        $this->open_xml_writer("activities/areview_{$this->moduleid}/inforef.xml");
