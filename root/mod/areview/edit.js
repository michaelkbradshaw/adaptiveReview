...
/**
 * JavaScript library for the areview module editing interface.
 *
 * @package    mod
 * @subpackage areview
...
// Initialise everything on the areview edit/order and paging page.
var areview_edit = {};
function areview_edit_init(Y) {
    ...
    areview_edit.randomquestiondialog = new Y.YUI2.widget.Dialog('randomquestiondialog', {
            modal: true,
            width: '100%',
            iframe: true,
            zIndex: 1000, // zIndex must be way above 99 to be above the active areview tab
            fixedcenter: true,
            visible: false,
            close: true,
    ...
    areview_edit.randomquestiondialog.render();
    ...
    Y.YUI2.util.Event.addListener(areview_edit_config.dialoglisteners, 'click', function(e) {
        ...
        areview_edit.randomquestiondialog.show();
        Y.YUI2.util.Event.stopEvent(e);
    });

    // Make escape close the dialogue.
    areview_edit.randomquestiondialog.cfg.setProperty('keylisteners', [new Y.YUI2.util.KeyListener(
            document, {keys:[27]}, function(types, args, obj) { areview_edit.randomquestiondialog.hide();
    })]);

    // Make the form cancel button close the dialogue.
    Y.YUI2.util.Event.addListener('id_cancel', 'click', function(e) {
        areview_edit.randomquestiondialog.hide();
        Y.YUI2.util.Event.preventDefault(e);
    });

    Y.YUI2.util.Event.addListener('id_existingcategory', 'click', areview_yui_workaround);

    Y.YUI2.util.Event.addListener('id_newcategory', 'click', areview_yui_workaround);

    // Repaginate dialogue -----------------------------------------------------
    areview_edit.repaginatedialog = new Y.YUI2.widget.Dialog('repaginatedialog', {
            modal: true,
            width: '30em',
            iframe: true,
            zIndex: 1000,
    ...
    areview_edit.repaginatedialog.render();
    areview_edit.randomquestiondialog.render();
    ...
    Y.YUI2.util.Event.addListener('repaginatecommand', 'click', function() {
        areview_edit.repaginatedialog.show();
    });
    ...
    Y.YUI2.widget.Overlay.windowResizeEvent.subscribe(function() {
      areview_edit.repaginatedialog.cfg.setProperty('context', ['repaginatecommand', 'tr', 'br', ['beforeShow']]);
    });

    // Make escape close the dialogue.
    areview_edit.repaginatedialog.cfg.setProperty('keylisteners', [new Y.YUI2.util.KeyListener(
            document, {keys:[27]}, function(types, args, obj) { areview_edit.repaginatedialog.hide();
    })]);
...
function areview_yui_workaround(e) {
...
// Initialise everything on the areview settings form.
function areview_settings_init(Y) {
    var repaginatecheckbox = document.getElementById('id_repaginatenow');
    if (!repaginatecheckbox) {
        // This checkbox does not appear on the create new areview form.
        return;
    }
