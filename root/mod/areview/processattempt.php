...
/**
 * This page deals with processing responses during an attempt at a areview.
 *
...
// to show the student another page of the areview. Just finish now.
$graceperiodmin = null;
...
$toolate = false;
if ($timeclose !== false && $timenow > $timeclose - areview_MIN_TIME_TO_CONTINUE) {
    $timeup = true;
...
if ($attemptobj->is_finished()) {
    throw new moodle_areview_exception($attemptobj->get_areviewobj(),
...
if ($timeup) {
    if ($attemptobj->get_areview()->overduehandling == 'graceperiod') {
        if (is_null($graceperiodmin)) {
            $graceperiodmin = get_config('areview', 'graceperiodmin');
        }
        if ($timenow > $timeclose + $attemptobj->get_areview()->graceperiod + $graceperiodmin) {
...
add_to_log($attemptobj->get_courseid(), 'areview', 'close attempt',
        'review.php?attempt=' . $attemptobj->get_attemptid(),
        $attemptobj->get_areviewid(), $attemptobj->get_cmid());

// Update the areview attempt record.
try {
