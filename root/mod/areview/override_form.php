...
/**
 * Settings form for overrides in the areview module.
 *
 * @package    mod
 * @subpackage areview
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class areview_override_form extends moodleform {

    /** @var object course module object. */
    protected $cm;

    /** @var object the areview settings object. */
    protected $areview;
     ...
     * @param object the areview settings object.
     ...
     */
    public function __construct($submiturl, $cm, $areview, $context, $groupmode, $override) {
    ...
    protected function definition() {
        ...
        $mform->addElement('header', 'override', get_string('override', 'areview'));
        ...
        if ($this->groupmode) {
            ...
            if ($this->groupid) {
                ...
                $mform->addElement('select', 'groupid',
                        get_string('overridegroup', 'areview'), $groupchoices);
                ...
                if (empty($groups)) {
                    // Generate an error.
                    $link = new moodle_url('/mod/areview/overrides.php', array('cmid'=>$cm->id));
                    print_error('groupsnone', 'areview', $link);
                }
            ...
            if ($this->userid) {
                ...
                $mform->addElement('select', 'userid',
                        get_string('overrideuser', 'areview'), $userchoices);
                $mform->freeze('userid');
            } else {
                ...
                if (!empty($CFG->enablegroupmembersonly) && $cm->groupmembersonly) {
                    ...
                    if (!empty($groups)) {
                        $users = get_users_by_capability($this->context, 'mod/areview:attempt',
                ...
                if (empty($users)) {
                    // Generate an error.
                    $link = new moodle_url('/mod/areview/overrides.php', array('cmid'=>$cm->id));
                    print_error('usersnone', 'areview', $link);
                }
        ...
        $mform->addElement('passwordunmask', 'password', get_string('requirepassword', 'areview'));
        $mform->setType('password', PARAM_TEXT);
        $mform->addHelpButton('password', 'requirepassword', 'areview');
        $mform->setDefault('password', $this->areview->password);

        // Open and close dates.
        $mform->addElement('date_time_selector', 'timeopen',
                get_string('areviewopen', 'areview'), array('optional' => true));
        $mform->setDefault('timeopen', $this->areview->timeopen);

        $mform->addElement('date_time_selector', 'timeclose',
                get_string('areviewclose', 'areview'), array('optional' => true));
        $mform->setDefault('timeclose', $this->areview->timeclose);

        // Time limit.
        $mform->addElement('duration', 'timelimit',
                get_string('timelimit', 'areview'), array('optional' => true));
        $mform->addHelpButton('timelimit', 'timelimit', 'areview');
        $mform->setDefault('timelimit', $this->areview->timelimit);
        ...
        $mform->addElement('select', 'attempts',
                get_string('attemptsallowed', 'areview'), $attemptoptions);
        $mform->setDefault('attempts', $this->areview->attempts);

        // Submit buttons.
        $mform->addElement('submit', 'resetbutton',
                get_string('reverttodefaults', 'areview'));

        $buttonarray = array();
        $buttonarray[] = $mform->createElement('submit', 'submitbutton',
                get_string('save', 'areview'));
        $buttonarray[] = $mform->createElement('submit', 'againbutton',
                get_string('saveoverrideandstay', 'areview'));
    ...
    public function validation($data, $files) {
        ...
        // Ensure that at least one areview setting was changed.
        $changed = false;
        $keys = array('timeopen', 'timeclose', 'timelimit', 'attempts', 'password');
        foreach ($keys as $key) {
            if ($data[$key] != $areview->{$key}) {
                $changed = true;
                break;
            }
        }
        if (!$changed) {
            $errors['timeopen'] = get_string('nooverridedata', 'areview');
        }
