...
/**
 * JavaScript library for the cquiz module editing interface.
 *
 * @package    mod
 * @subpackage cquiz
...
// Initialise everything on the cquiz edit/order and paging page.
var cquiz_edit = {};
function cquiz_edit_init(Y) {
    ...
    cquiz_edit.randomquestiondialog = new Y.YUI2.widget.Dialog('randomquestiondialog', {
            modal: true,
            width: '100%',
            iframe: true,
            zIndex: 1000, // zIndex must be way above 99 to be above the active cquiz tab
            fixedcenter: true,
            visible: false,
            close: true,
    ...
    cquiz_edit.randomquestiondialog.render();
    ...
    Y.YUI2.util.Event.addListener(cquiz_edit_config.dialoglisteners, 'click', function(e) {
        ...
        cquiz_edit.randomquestiondialog.show();
        Y.YUI2.util.Event.stopEvent(e);
    });

    // Make escape close the dialogue.
    cquiz_edit.randomquestiondialog.cfg.setProperty('keylisteners', [new Y.YUI2.util.KeyListener(
            document, {keys:[27]}, function(types, args, obj) { cquiz_edit.randomquestiondialog.hide();
    })]);

    // Make the form cancel button close the dialogue.
    Y.YUI2.util.Event.addListener('id_cancel', 'click', function(e) {
        cquiz_edit.randomquestiondialog.hide();
        Y.YUI2.util.Event.preventDefault(e);
    });

    Y.YUI2.util.Event.addListener('id_existingcategory', 'click', cquiz_yui_workaround);

    Y.YUI2.util.Event.addListener('id_newcategory', 'click', cquiz_yui_workaround);

    // Repaginate dialogue -----------------------------------------------------
    cquiz_edit.repaginatedialog = new Y.YUI2.widget.Dialog('repaginatedialog', {
            modal: true,
            width: '30em',
            iframe: true,
            zIndex: 1000,
    ...
    cquiz_edit.repaginatedialog.render();
    cquiz_edit.randomquestiondialog.render();
    ...
    Y.YUI2.util.Event.addListener('repaginatecommand', 'click', function() {
        cquiz_edit.repaginatedialog.show();
    });
    ...
    Y.YUI2.widget.Overlay.windowResizeEvent.subscribe(function() {
      cquiz_edit.repaginatedialog.cfg.setProperty('context', ['repaginatecommand', 'tr', 'br', ['beforeShow']]);
    });

    // Make escape close the dialogue.
    cquiz_edit.repaginatedialog.cfg.setProperty('keylisteners', [new Y.YUI2.util.KeyListener(
            document, {keys:[27]}, function(types, args, obj) { cquiz_edit.repaginatedialog.hide();
    })]);
...
function cquiz_yui_workaround(e) {
...
// Initialise everything on the cquiz settings form.
function cquiz_settings_init(Y) {
    var repaginatecheckbox = document.getElementById('id_repaginatenow');
    if (!repaginatecheckbox) {
        // This checkbox does not appear on the create new cquiz form.
        return;
    }
