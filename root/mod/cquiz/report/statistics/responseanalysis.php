 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class cquiz_statistics_response_analyser {
    ...
    /**
     * Store the computed response analysis in the cquiz_quest_response_stats
     * table.
     * @param int $cquizstatisticsid the cached cquiz statistics to load the
     * data corresponding to.
     * @return bool true if cached data was found in the database and loaded,
     * otherwise false, to mean no data was loaded.
     */
    public function load_cached($cquizstatisticsid) {
        global $DB;

        $rows = $DB->get_records('cquiz_quest_response_stats',
                array('cquizstatisticsid' => $cquizstatisticsid,
     ...
     * Store the computed response analysis in the cquiz_quest_response_stats
     * table.
     * @param int $cquizstatisticsid the cached cquiz statistics this correspons to.
     */
    public function store_cached($cquizstatisticsid) {
        global $DB;
        ...
        foreach ($this->responses as $subpartid => $partdata) {
            foreach ($partdata as $responseclassid => $classdata) {
                foreach ($classdata as $response => $data) {
                    $row = new stdClass();
                    $row->cquizstatisticsid = $cquizstatisticsid;
                    ...
                    $DB->insert_record('cquiz_quest_response_stats', $row, false);
                }
            }
        }
    }
