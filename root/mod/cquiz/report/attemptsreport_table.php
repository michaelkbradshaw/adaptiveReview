...
abstract class cquiz_attempts_report_table extends table_sql {
     ...
     */
    public function __construct($uniqueid, $cquiz, $context, $qmsubselect,
            mod_cquiz_attempts_report_options $options, $groupstudents, $students,
            $questions, $reporturl) {
    ...
    public function col_fullname($attempt) {
        ...
        return $html . html_writer::empty_tag('br') . html_writer::link(
                new moodle_url('/mod/cquiz/review.php', array('attempt' => $attempt->attempt)),
                get_string('reviewattempt', 'cquiz'), array('class' => 'reviewlink'));
    }
    ...
    public function col_feedbacktext($attempt) {
        ...
        $feedback = cquiz_report_feedback_for_grade(
                cquiz_rescale_grade($attempt->sumgrades, $this->cquiz, false),
                $this->cquiz->id, $this->context);
    ...
    protected function add_latest_state_join($slot) {
        ...
        $qubaids = new qubaid_join("{cquiz_attempts} {$alias}cquiza", "{$alias}cquiza.uniqueid",
                "{$alias}cquiza.cquiz = :{$alias}cquizid", array("{$alias}cquizid" => $this->sql->params['cquizid']));
    ...
    protected function get_qubaids_condition() {
        ...
        if ($this->is_downloading()) {
            ...
            return new qubaid_join($this->sql->from, 'cquiza.uniqueid',
    ...
    protected function submit_buttons() {
        ...
        if (has_capability('mod/cquiz:deleteattempts', $this->context)) {
            ...
            $PAGE->requires->event_handler('#deleteattemptsbutton', 'click', 'M.util.show_confirm_dialog',
                    array('message' => get_string('deleteattemptcheck', 'cquiz')));
        }
    }
}
