...
class cquiz_grading_report extends cquiz_default_report {
    ...
    public function display($cquiz, $cm, $course) {
        ...
        $questionsincquiz = cquiz_questions_in_cquiz($cquiz->questions);
        $counts = null;
        if ($slot && $questionsincquiz) {
    ...
    protected function get_qubaids_condition() {
        ...
        if ($currentgroup) {
            $users = get_users_by_capability($this->context,
                    array('mod/cquiz:reviewmyattempts', 'mod/cquiz:attempt'), 'u.id, u.id', '', '', '',
                    $currentgroup, '', false);
            if (empty($users)) {
    ...
    protected function load_attempts_by_usage_ids($qubaids) {
        ...
        $attemptsbyid = $DB->get_records_sql("
                SELECT cquiza.*, u.firstname, u.lastname, u.idnumber
                FROM {cquiz_attempts} cquiza
                JOIN {user} u ON u.id = cquiza.userid
                WHERE cquiza.uniqueid $asql AND cquiza.state = ? AND cquiza.cquiz = ?",
                $params);
    ...
    protected function base_url() {
        return new moodle_url('/mod/cquiz/report.php',
    ...
    protected function format_count_for_table($counts, $type, $gradestring) {
        ...
        if ($counts->$type > 0) {
            $result .= ' ' . html_writer::link($this->grade_question_url(
                    ...
                    get_string($gradestring, 'cquiz_grading'),
    ...
    protected function display_index($includeauto) {
        ...
        echo $OUTPUT->heading(get_string('questionsthatneedgrading', 'cquiz_grading'));
        if ($includeauto) {
        ...
        if (empty($data)) {
            echo $OUTPUT->heading(get_string('nothingfound', 'cquiz_grading'));
            return;
        }
    ...
    protected function display_grading_interface($slot, $questionid, $grade,
        ...
        echo $OUTPUT->heading(get_string('gradingquestionx', 'cquiz_grading', $a));
        echo html_writer::tag('p', html_writer::link($this->list_questions_url(),
                get_string('backtothelistofquestions', 'cquiz_grading')),
        ...
        echo html_writer::tag('div', html_writer::empty_tag('input', array(
                'type' => 'submit', 'value' => get_string('saveandnext', 'cquiz_grading'))),
                array('class' => 'mdl-align')) .
