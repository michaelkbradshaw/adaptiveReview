...
list($thispageurl, $contexts, $cmid, $cm, $cquiz, $pagevars) =
        question_edit_setup('editq', '/mod/cquiz/addrandom.php', true);
...
$mform = new cquiz_add_random_form(new moodle_url('/mod/cquiz/addrandom.php'), $contexts);
...
if ($data = $mform->get_data()) {
    if (!empty($data->existingcategory)) {
        ...
        add_to_log($cquiz->course, 'cquiz', 'addcategory',
    ...
    cquiz_add_random_questions($cquiz, $addonpage, $categoryid, 1, $includesubcategories);
...
echo $OUTPUT->heading(get_string('addrandomquestiontocquiz', 'cquiz', $cquizname), 2, 'mdl-left');
$mform->display();
echo $OUTPUT->footer();
