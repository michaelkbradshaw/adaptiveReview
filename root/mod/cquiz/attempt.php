...
if ($id = optional_param('id', 0, PARAM_INT)) {
    redirect($CFG->wwwroot . '/mod/cquiz/startattempt.php?cmid=' . $id . '&sesskey=' . sesskey());
} else if ($qid = optional_param('q', 0, PARAM_INT)) {
    if (!$cm = get_coursemodule_from_instance('cquiz', $qid)) {
        print_error('invalidcquizid', 'cquiz');
    }
    redirect(new moodle_url('/mod/cquiz/startattempt.php',
...
if (!$attemptobj->is_preview_user() && $messages) {
    print_error('attempterror', 'cquiz', $attemptobj->view_url(),
...
if ($autosaveperiod) {
    $PAGE->requires->yui_module('moodle-mod_cquiz-autosave',
            'M.mod_cquiz.autosave.init', array($autosaveperiod));
}

// Log this page view.
add_to_log($attemptobj->get_courseid(), 'cquiz', 'continue attempt',
        'review.php?attempt=' . $attemptobj->get_attemptid(),
        $attemptobj->get_cquizid(), $attemptobj->get_cmid());
...
if ($attemptobj->get_currentpage() != $page) {
    if ($attemptobj->get_navigation_method() == CQUIZ_NAVMETHOD_SEQ && $attemptobj->get_currentpage() > $page) {
        // Prevent out of sequence access.
        redirect($attemptobj->start_attempt_url(null, $attemptobj->get_currentpage()));
    }
    $DB->set_field('cquiz_attempts', 'currentpage', $page, array('id' => $attemptid));
}
...
$headtags = $attemptobj->get_html_head_contributions($page);
$PAGE->set_title(format_string($attemptobj->get_cquiz_name()));
