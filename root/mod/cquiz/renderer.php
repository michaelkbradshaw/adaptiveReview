...
class mod_cquiz_renderer extends plugin_renderer_base {
     ...
     */
    public function review_page(cquiz_attempt $attemptobj, $slots, $page, $showall,
                                $lastpage, mod_cquiz_display_options $displayoptions,
                                $summarydata) {
     ...
     * @return $output containing html data.
     */
    public function review_question_page(cquiz_attempt $attemptobj, $slot, $seq,
            mod_cquiz_display_options $displayoptions, $summarydata) {
    ...
    public function review_summary_table($summarydata, $page) {
        ...
        $output .= html_writer::start_tag('table', array(
                'class' => 'generaltable generalbox cquizreviewsummary'));
     ...
     * @param mod_cquiz_display_options $displayoptions instance of mod_cquiz_display_options
     */
    public function questions(cquiz_attempt $attemptobj, $reviewing, $slots, $page, $showall,
                              mod_cquiz_display_options $displayoptions) {
        $output = '';
    ...
    public function review_form($page, $showall, $displayoptions, $content, $attemptobj) {
        ...
        $this->page->requires->js_init_call('M.mod_cquiz.init_review_form', null, false,
                cquiz_get_js_module());
    ...
    public function finish_review_link(cquiz_attempt $attemptobj) {
        ...
        if ($attemptobj->get_access_manager(time())->attempt_must_be_in_popup()) {
            $this->page->requires->js_init_call('M.mod_cquiz.secure_window.init_close_button',
                    array($url), cquiz_get_js_module());
            return html_writer::empty_tag('input', array('type' => 'button',
                    'value' => get_string('finishreview', 'cquiz'),
                    'id' => 'secureclosebutton'));

        } else {
            return html_writer::link($url, get_string('finishreview', 'cquiz'));
        }
    }
     ...
     */
    public function review_next_navigation(cquiz_attempt $attemptobj, $page, $lastpage) {
        if ($lastpage) {
     ...
     */
    public function countdown_timer(cquiz_attempt $attemptobj, $timenow) {
        ...
        return html_writer::tag('div', get_string('timeleft', 'cquiz') . ' ' .
                html_writer::tag('span', '', array('id' => 'cquiz-time-left')),
                array('id' => 'cquiz-timer', 'role' => 'timer',
    ...
    public function navigation_panel(cquiz_nav_panel_base $panel) {
        ...
        $this->page->requires->js_init_call('M.mod_cquiz.nav.init', null, false,
                cquiz_get_js_module());
    ...
    /**
     * Returns the cquizzes navigation button
     *
    ...
    protected function render_cquiz_nav_question_button(cquiz_nav_question_button $button) {
        ...
        if ($button->currentpage) {
            $classes[] = 'thispage';
            $attributes[] = get_string('onthispage', 'cquiz');
        }
    ...
    public function close_attempt_popup($url, $message = '') {
        ...
        $this->page->requires->js_init_call('M.mod_cquiz.secure_window.close',
                array($url, $delay), false, cquiz_get_js_module());
    ...
    public function summary_page_controls($attemptobj) {
        ...
        if ($attemptobj->get_state() == cquiz_attempt::IN_PROGRESS) {
            $button = new single_button(
                    ...
                    get_string('returnattempt', 'cquiz'));
        ...
        $button = new single_button(
                ...
                get_string('submitallandfinish', 'cquiz'));
        $button->id = 'responseform';
        if ($attemptobj->get_state() == cquiz_attempt::IN_PROGRESS) {
            $button->add_action(new confirm_action(get_string('confirmclose', 'cquiz'), null,
                    get_string('submitallandfinish', 'cquiz')));
        }

        $duedate = $attemptobj->get_due_date();
        $message = '';
        if ($attemptobj->get_state() == cquiz_attempt::OVERDUE) {
            $message = get_string('overduemustbesubmittedby', 'cquiz', userdate($duedate));

        } else if ($duedate) {
            $message = get_string('mustbesubmittedby', 'cquiz', userdate($duedate));
        }
     ...
     * @param array $infomessages information about this cquiz
     ...
     *      attempt this cquiz now, if appicable this cquiz
     */
    public function view_page($course, $cquiz, $cm, $context, $viewobj) {
        $output = '';
        ...
        $output .= $this->box($this->view_page_buttons($viewobj), 'cquizattempt');
        return $output;
    }
    ...
    public function view_page_buttons(mod_cquiz_view_object $viewobj) {
        ...
        if ($viewobj->buttontext) {
            ...
            $output .= $this->single_button($viewobj->backtocourseurl,
                    get_string('backtocourse', 'cquiz'), 'get',
     ...
     * @param array $cquiz Array containging cquiz date
    ...
    public function start_attempt_button($buttontext, moodle_url $url,
        ...
        $warning = '';
        if ($popuprequired) {
            $this->page->requires->js_module(cquiz_get_js_module());
            $this->page->requires->js('/mod/cquiz/module.js');
            $popupaction = new popup_action('click', $url, 'cquizpopup', $popupoptions);

            $button->class .= ' cquizsecuremoderequired';
            $button->add_action(new component_action('click',
                    'M.mod_cquiz.secure_window.start_attempt_action', array(
                        'url' => $url->out(false),
            ...
            $warning = html_writer::tag('noscript', $this->heading(get_string('noscript', 'cquiz')));

        } else if ($startattemptwarning) {
            $button->add_action(new confirm_action($startattemptwarning, null,
                    get_string('startattempt', 'cquiz')));
        }
    ...
    public function view_information($cquiz, $cm, $context, $messages) {
        ...
        // Print cquiz name and description.
        $output .= $this->heading(format_string($cquiz->name));
        $output .= $this->cquiz_intro($cquiz, $cm);

        // Output any access messages.
        if ($messages) {
            $output .= $this->box($this->access_messages($messages), 'cquizinfo');
        }

        // Show number of attempts summary to those who can view reports.
        if (has_capability('mod/cquiz:viewreports', $context)) {
            if ($strattemptnum = $this->cquiz_attempt_summary_link_to_reports($cquiz, $cm,
                    $context)) {
                $output .= html_writer::tag('div', $strattemptnum,
                        array('class' => 'cquizattemptcounts'));
            }
        }
        return $output;
    }
    ...
    public function view_table_heading() {
        return $this->heading(get_string('summaryofattempts', 'cquiz'));
    }
    ...
    public function view_table($cquiz, $context, $viewobj) {
        ...
        $table = new html_table();
        $table->attributes['class'] = 'generaltable cquizattemptsummary';
        $table->head = array();
        $table->align = array();
        $table->size = array();
...
//            $attemptgrade = $attemptobj->get_sum_marks()/
  //          $attemptobj->get_num_questions();
    ...
    public function attempt_state($attemptobj) {
        switch ($attemptobj->get_state()) {
            ...
            case cquiz_attempt::OVERDUE:
                return get_string('stateoverdue', 'cquiz') . html_writer::tag('span',
                        get_string('stateoverduedetails', 'cquiz',
                                userdate($attemptobj->get_due_date())),
                        array('class' => 'statedetails'));

            case cquiz_attempt::FINISHED:
                return get_string('statefinished', 'cquiz') . html_writer::tag('span',
                        get_string('statefinisheddetails', 'cquiz',
    ...
    public function view_best_score($viewobj) {
        $output = '';
        // Print information about the student's best score for this cquiz if possible.
        if (!$viewobj->moreattempts) {
            $output .= $this->heading(get_string('nomoreattempts', 'cquiz'));
        }
        return $output;
    }
    ...
    public function view_result_info($cquiz, $context, $cm, $viewobj) {
        ...
        if ($viewobj->overallstats) {
            if ($viewobj->moreattempts) {
                ...
                $resultinfo .= $this->heading(get_string('gradesofar', 'cquiz', $a), 2, 'main');
            } else {
                $a = new stdClass();
                ...
                $resultinfo .= $this->heading(get_string('yourfinalgradeis', 'cquiz', $a), 2,
                        'main');
            }
        }
        ...
        if ($viewobj->gradebookfeedback) {
            $resultinfo .= $this->heading(get_string('comment', 'cquiz'), 3, 'main');
            $resultinfo .= '<p class="cquizteacherfeedback">'.$viewobj->gradebookfeedback.
                    "</p>\n";
        }
        if ($viewobj->feedbackcolumn) {
            $resultinfo .= $this->heading(get_string('overallfeedback', 'cquiz'), 3, 'main');
            $resultinfo .= html_writer::tag('p',
                    cquiz_feedback_for_grade($viewobj->mygrade, $cquiz, $context),
                    array('class' => 'cquizgradefeedback'))."\n";
        }
    ...
    public function review_link($url, $reviewinpopup, $popupoptions) {
        if ($reviewinpopup) {
            ...
            return html_writer::link($url, get_string('review', 'cquiz'),
                    array('title' => get_string('reviewthisattempt', 'cquiz')));
        }
    }
     ...
     */
    public function cquiz_attempt_summary_link_to_reports($cquiz, $cm, $context,
