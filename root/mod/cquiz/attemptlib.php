...
//    	print_object(debug_backtrace());
    	...
    	*/
        ...
        //grab existing scores
       	$scores = cquiz_get_cummulative_scores($cquiz->id,$USER->id);
...
ABC;
     ...
     */
    public static function create($cquizid, $userid) {
        global $DB;
    ...
    public function preload_questions() {
        if (empty($this->questionids)) {
            throw new moodle_cquiz_exception($this, 'noquestions', $this->edit_url());
        }
        ...
        $this->questions = question_preload_questions($this->questionids,
				...
				$from,
        		array('courseid' => $this->cquiz->course));
        
        
      /*  $this->questions = question_preload_questions($this->questionids,
        		'1 AS maxmark, 1 AS instance',
        		'',
        		array('cquizid' => $this->cquiz->id));
        		*/
        /*
         * there will not be a question instance for the questions in the quiz
        $this->questions = question_preload_questions($this->questionids,
        		'qqi.grade AS maxmark, qqi.id AS instance', //selects
        		'{cquiz_question_instances} qqi ON qqi.cquiz = :cquizid AND q.id = qqi.question', //Froms
        		array('cquizid' => $this->cquiz->id)); //Vars
        */
    ...
    public function get_access_manager($timenow) {
        if (is_null($this->accessmanager)) {
            $this->accessmanager = new cquiz_access_manager($this, $timenow,
                    has_capability('mod/cquiz:ignoretimelimits', $this->context, null, false));
        }
     ...
     * @return string if the cquiz policies merit it, return a warning string to
    ...
    public function confirm_start_attempt_message($unfinished) {
        ...
        if ($this->cquiz->timelimit && $this->cquiz->attempts) {
            return get_string('confirmstartattempttimelimit', 'cquiz', $this->cquiz->attempts);
        } else if ($this->cquiz->timelimit) {
            return get_string('confirmstarttimelimit', 'cquiz');
        } else if ($this->cquiz->attempts) {
            return get_string('confirmstartattemptlimit', 'cquiz', $this->cquiz->attempts);
        }
    ...
    public function cannot_review_message($when, $short = false) {
        ...
        if ($when == mod_cquiz_display_options::DURING ||
            ...
            return get_string('noreviewuntil' . $langstrsuffix, 'cquiz',
                    userdate($this->cquiz->timeclose, $dateformat));
        } else {
...
class cquiz_attempt {
    ...
    private function determine_layout() {
        ...
        $pagelayouts = explode(',0', cquiz_clean_layout($this->attempt->layout, true));
     ...
     * or someone with 'mod/cquiz:viewreports' reviewing someone elses attempt.
     *
    ...
    public function get_overall_feedback($grade) {
        return cquiz_feedback_for_grade($grade, $this->get_cquiz(),
                $this->cquizobj->get_context());
    }
    ...
    public function check_review_capability() {
        if (!$this->has_capability('mod/cquiz:viewreports')) {
            if ($this->get_attempt_state() == mod_cquiz_display_options::IMMEDIATELY_AFTER) {
                $this->require_capability('mod/cquiz:attempt');
            } else {
                $this->require_capability('mod/cquiz:reviewmyattempts');
            }
        }
    }
    ...
    public function get_display_options($reviewing) {
        if ($reviewing) {
            if (is_null($this->reviewoptions)) {
                $this->reviewoptions = cquiz_get_review_options($this->get_cquiz(),
                        $this->attempt, $this->cquizobj->get_context());
            }
            return $this->reviewoptions;

        } else {
            $options = mod_cquiz_display_options::make_from_cquiz($this->get_cquiz(),
    ...
    /**
     * Return the list of question ids for either a given page of the cquiz, or for the
     * whole cquiz.
     *
    ...
    public function start_attempt_url($slot = null, $page = -1) {
        ...
        return $this->cquizobj->start_attempt_url($page);
    }
    ...
    public function restart_preview_button() {
        ...
        if ($this->is_preview() && $this->is_preview_user()) {
            return $OUTPUT->single_button(new moodle_url(
                    ...
                    get_string('startnewpreview', 'cquiz'));
        } else {
            return '';
        }
    }
     ...
     */
    public function get_navigation_panel(mod_cquiz_renderer $output,
        ...
        $bc->title = get_string('cquiznavigation', 'cquiz');
    ...
    public function process_finish($timestamp, $processsubmitted) {
        ...
        if (!$this->is_preview()) {
            ...
            // Trigger event.
            $this->fire_state_transition_event('cquiz_attempt_submitted', $timestamp);
    ...
    public function process_going_overdue($timestamp, $studentisonline) {
        ...
        $this->fire_state_transition_event('cquiz_attempt_overdue', $timestamp);
    ...
    public function process_abandon($timestamp, $studentisonline) {
        ...
        $this->fire_state_transition_event('cquiz_attempt_abandoned', $timestamp);
    ...
    protected function fire_state_transition_event($event, $timestamp) {
        ...
        $eventdata->cquizid      = $this->get_cquizid();
        ...
        if ($event == 'cquiz_attempt_submitted') {
    ...
    public function question_print_comment_fields($slot, $prefix) {
        ...
        question_print_comment_fields($this->quba->get_question_attempt($slot),
                ...
                get_string('gradingattempt', 'cquiz_grading', $a));
    }
    ...
    protected function page_and_question_url($script, $slot, $page, $showall, $thispage) {
        ...
        if ($thispage == $page) {
            ...
            $url = new moodle_url('/mod/cquiz/' . $script . '.php' . $fragment,
...
abstract class cquiz_nav_panel_base {
    ...
    public function __construct(cquiz_attempt $attemptobj,
    ...
    public function user_picture() {
        ...
        if (!$this->attemptobj->get_cquiz()->showuserpicture) {
            return null;
        }
...
class cquiz_attempt_nav_panel extends cquiz_nav_panel_base {
    ...
    public function render_before_button_bits(mod_cquiz_renderer $output) {
        return html_writer::tag('div', get_string('navnojswarning', 'cquiz'),
                array('id' => 'cquiznojswarning'));
    }

    public function render_end_bits(mod_cquiz_renderer $output) {
        return html_writer::link($this->attemptobj->summary_url(),
                get_string('endtest', 'cquiz'), array('class' => 'endtestlink')) .
