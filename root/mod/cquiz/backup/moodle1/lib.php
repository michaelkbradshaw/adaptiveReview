 ...
 */
class moodle1_mod_cquiz_handler extends moodle1_mod_handler {
    ...
    public function get_paths() {
        return array(
            new convert_path(
                'cquiz', '/MOODLE_BACKUP/COURSE/MODULES/MOD/QUIZ',
                array(
                    'newfields' => array(
                        'showuserpicture'   => 0,
     ...
     * data available
     */
    public function process_cquiz($data) {
        global $CFG;

        // Replay the upgrade step 2008081501.
        if (is_null($data['sumgrades'])) {
            $data['sumgrades'] = 0;
            // TODO for user data: cquiz_attempts SET sumgrades=0 WHERE sumgrades IS NULL.
            // TODO for user data: cquiz_grades.grade should be not be null , convert to default 0.
        }
        ...
        $this->fileman = $this->converter->get_file_manager($contextid, 'mod_cquiz');
        ...
        // Start writing cquiz.xml.
        $this->open_xml_writer("activities/cquiz_{$this->moduleid}/cquiz.xml");
        $this->xmlwriter->begin_tag('activity', array('id' => $instanceid,
                'moduleid' => $this->moduleid, 'modulename' => 'cquiz',
                'contextid' => $contextid));
        $this->xmlwriter->begin_tag('cquiz', array('id' => $instanceid));
    ...
    public function on_cquiz_question_instances_start() {
        $this->xmlwriter->begin_tag('question_instances');
    }

    public function on_cquiz_question_instances_end() {
        $this->xmlwriter->end_tag('question_instances');
    }

    public function process_cquiz_question_instance($data) {
        $this->write_xml('question_instance', $data, array('/question_instance/id'));
    }

    public function on_cquiz_feedbacks_start() {
        $this->xmlwriter->begin_tag('feedbacks');
    }

    public function on_cquiz_feedbacks_end() {
        $this->xmlwriter->end_tag('feedbacks');
    }

    public function process_cquiz_feedback($data) {
    ...
    /**
     * This is executed when we reach the closing </MOD> tag of our 'cquiz' path
     */
    public function on_cquiz_end() {

        // Append empty <overrides> subpath element.
        $this->write_xml('overrides', array());

        // Finish writing cquiz.xml.
        $this->xmlwriter->end_tag('cquiz');
        $this->xmlwriter->end_tag('activity');
        $this->close_xml_writer();

        // Write inforef.xml.
        $this->open_xml_writer("activities/cquiz_{$this->moduleid}/inforef.xml");
