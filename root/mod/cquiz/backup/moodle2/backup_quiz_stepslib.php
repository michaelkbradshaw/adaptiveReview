...
class backup_cquiz_activity_structure_step extends backup_questions_activity_structure_step {
    ...
    protected function define_structure() {
        ...
        $qinstance->set_source_table('cquiz_question_instances',
                array('cquiz' => backup::VAR_PARENTID));

        $feedback->set_source_table('cquiz_feedback',
                array('cquizid' => backup::VAR_PARENTID));
        ...
        if ($userinfo) {
            ...
            $attempt->set_source_sql('
                    SELECT *
                    FROM {cquiz_attempts}
                    WHERE cquiz = :cquiz AND preview = 0',
                    array('cquiz' => backup::VAR_PARENTID));
        }
